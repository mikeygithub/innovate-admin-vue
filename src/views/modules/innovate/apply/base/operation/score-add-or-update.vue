<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="8rem">
      ...
      <el-form-item label="意见反馈" prop="opinion">
        <el-input
          type="textarea"
          :rows="2"
          placeholder="请输入"
    ...
    methods: {
      init (index) {
        ...
        this.$nextTick(() => {
          ...
          if (this.id) {
            this.$http({
              url: this.$http.adornUrl(`/innovate/project/review/info`),
              method: 'get',
              params: this.$http.adornParams({
                'apply': this.dataForm.apply,
                'userId': this.$store.state.user.id,
                'projectId': this.id,
                'isDel': 0
              })
              ...
              if (data && data.code === 0) {
                this.dataForm = data.projectReviewEntity
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/innovate/project/review/score`),
              method: 'post',
