<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="150px">
      <el-form-item label="评委组" prop="matchInfoEntity.groupId">
        <el-select v-model="dataForm.matchInfoEntity.groupId" placeholder="请选择">
          <el-option
    ...
    <el-card>
      当前评委组人员：
      <template v-for="item in groupList" v-if="item.groupId === dataForm.matchInfoEntity.groupId">
        <template v-for="groupUser in item.innovateReviewGroupUserEntities">
          <template v-for="user in sysUserEntities" v-if="groupUser.userId === user.userId">
            {{user.name}}
            <el-tag v-for="teacher in dataForm.matchTeacherEntities"
                    :key="teacher.matchTeacherId"
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/innovate/match/review/review`),
              method: 'post',
              data: this.$http.adornData({
                'groupId': this.dataForm.matchInfoEntity.groupId,
                'apply': 'project_match_apply_status',
                'matchId': this.id,
                'roleId': 5,
                'userId': this.userId,
