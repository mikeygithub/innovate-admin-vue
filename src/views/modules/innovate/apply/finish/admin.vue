    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyIsVisible(scope.row.finishInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.finishInfoEntity.finishId)">分配评委组</el-button>
          <el-button v-if="reApplyIsVisible(scope.row.finishInfoEntity)" type="text" size="small" @click="reviewAddOrUpdateHandle(scope.row.finishInfoEntity.finishId)">重新分配评委组</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      canUpdateVisible (item) {
        this.$http({
          url: this.$http.adornUrl(`/innovate/finish/apply/update/list`),
          method: 'post',
      ...
      retreatIsVisible (item) {
        if (this.isAuth('innovate:finish:retreat')) {
          if (item.projectFinishApplyStatus !== null || item.projectFinishApplyStatus !== '') {
            if (item.projectFinishApplyStatus === 3) {
