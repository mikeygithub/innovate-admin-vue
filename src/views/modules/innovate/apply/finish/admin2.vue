    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="applyFinishIsVisible(scope.row.finishInfoEntity)" type="text" size="small" @click="applyFinishHandle(scope.row.finishInfoEntity.finishId)">公布结题</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        // }).then(({data}) => {
        //   if (data && data.code === 0) {
        //     this.eventLists = data.finishEventEntityList
        //     this.$store.state.eventLists = data.finishEventEntityList
        //   }
        // })
        this.$http({
      ...
      applyFinishIsVisible (item) {
        if (this.isAuth('innovate:finish:apply:finish')) {
          if (item.projectFinishApplyStatus !== null || item.projectFinishApplyStatus !== '') {
            if (item.projectFinishApplyStatus === 5) {
      ...
      applyFinishHandle (id) {
        this.$confirm('此操作将使该项目公布结题，是否继续?', '提示', {
